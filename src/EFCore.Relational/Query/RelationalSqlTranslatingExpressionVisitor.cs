...
#pragma warning restore IDE0066 // Convert switch statement to expression
    ...
    private sealed class GroupByAggregateChainProcessor : ExpressionVisitor
    {
        ...
        protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
        {
                ...
                && methodCallExpression.Method.DeclaringType == typeof(Queryable))
            {
                ...
                {
                    return new GroupAggregetingElementExpression(groupByShaperExpression.ElementSelector);
                }
                ...
                if (source is GroupAggregetingElementExpression groupAggregatingElementExpression)
                {
                    ...
                    static void ProcessSelector(GroupAggregetingElementExpression groupingElement,
                        LambdaExpression lambdaExpression)
                    {
                        var selector = RemapLambda(groupingElement, lambdaExpression);

                        groupingElement.ApplySelector(selector);
                    }

                    static Expression RemapLambda(GroupAggregetingElementExpression groupingElement, LambdaExpression lambdaExpression)
                        => ReplacingExpressionVisitor.Replace(lambdaExpression.Parameters[0], groupingElement.Element, lambdaExpression.Body);

                    bool ProcessPredicate(GroupAggregetingElementExpression groupingElement, LambdaExpression lambdaExpression)
                    {
                        var lambdaBody = RemapLambda(groupingElement, lambdaExpression);
        ...
        private SqlExpression? TranslateAggregate(MethodInfo methodInfo, GroupAggregetingElementExpression groupAggregetingElementExpression)
        {
            var selector = _sqlTranslatingExpressionVisitor.TranslateInternal(groupAggregetingElementExpression.Element);
            if (selector == null)
            {
            ...
            if (groupAggregetingElementExpression.Predicate != null)
            {
                ...
                selector = _sqlTranslatingExpressionVisitor._sqlExpressionFactory.Case(
                    new List<CaseWhenClause> { new(groupAggregetingElementExpression.Predicate, selector) },
                    elseResult: null);
            }

            var sqlExpression = new SqlEnumerableExpression(selector, groupAggregetingElementExpression.IsDistinct, null);
    ...
    private sealed class GroupAggregetingElementExpression : Expression
    {
        public GroupAggregetingElementExpression(Expression element)
        {
            Element = element;
        }
        ...
        public GroupAggregetingElementExpression ApplyDistinct()
        {
            IsDistinct = true;
        ...
        public GroupAggregetingElementExpression ApplySelector(Expression expression)
        {
            Element = expression;
        ...
        public GroupAggregetingElementExpression ApplyPredicate(SqlExpression expression)
        {
